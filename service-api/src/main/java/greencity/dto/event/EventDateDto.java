...
public record EventDateDto(
    @NotNull(message = "Event date is required") @FutureOrPresentDate(
        message = "Event date cannot be in the past") @JsonFormat(pattern = "yyyy-MM-dd") LocalDate date,
    ...
    @URL(message = "Invalid URL format for online link") String onlineLink) {
    @JsonIgnore
