...
import static greencity.constant.RestTemplateLinks.USER_FIND_NAMES_BY_IDS;
...
public class RestClient {
    ...
    /**
     * Method that returns usernames by user ids.
     *
     * @param ids {@link Set} of user ids.
     * @return {@link Map} where key is user id and value is username.
     */
    public Map<Long, String> findUserNamesByUserIds(Set<Long> ids) {
        HttpHeaders headers = setHeader();
        headers.setContentType(MediaType.APPLICATION_JSON);
        HttpEntity<Set<Long>> entity = new HttpEntity<>(ids, headers);

        return restTemplate.exchange(greenCityUserServerAddress + USER_FIND_NAMES_BY_IDS,
            HttpMethod.POST,
            entity,
